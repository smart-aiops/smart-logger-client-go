...
func AcclogSetUp() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		data := proto.RawData{
			...
			Args:     "d",
			RespBody: "e",
			CostTime: (endTime.UnixNano() - startTime.UnixNano()) / 1000 / 1000,
			LogDate:  time.Now().Unix(),
			Success:  true,
			Account:  "g",
		}
		fmt.Printf("grpctrans send data : %v\n", &data)

		_, err := grpctrans.GrpcClientConf.Send(data)
		if err != nil {
			fmt.Printf("grpctrans send data failed, err: %v", err)
		}
	}
}
